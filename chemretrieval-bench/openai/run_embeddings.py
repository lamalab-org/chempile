...
def count_tokens(text: str, model_name: str) -> int:
    """
    Count the number of tokens in the text using the same encoding as the model.

    Args:
        text: Input text to count tokens for
        model_name: Name of the model to get the appropriate encoding
...
def split_text_by_tokens(
    text: str, model_name: str, max_tokens: int = 8000
) -> List[str]:
    """
    ...
    Args:
        ...
        model_name: Name of the model to get the appropriate encoding
        max_tokens: Maximum tokens per chunk (default 8000 to leave buffer)
...
def get_embedding(
    text: str, client=client, model_name: str = model_name, max_tokens: int = 8000
) -> np.ndarray:
    """
    Get embeddings for text, handling cases where text exceeds token limit.
    ...
    Returns:
        numpy array of embeddings (concatenated if text was split)
    """
    # Count tokens in the input text
    token_count = count_tokens(text, model_name)
    print(f"Input text has {token_count} tokens")

    # If text is within token limit, process normally
    ...
    text_chunks = split_text_by_tokens(text, model_name, max_tokens)
    ...
    print(f"Final embedding shape: {concatenated_embedding.shape}")
